...
-- -----------------------------------------------------------------------------
-- Nota: SUM() sobre entero devuelve numerico en PostgreSQL. Se convierte de vuelta a entero
-- explicitamente para que ninguna columna expuesta sea decimal. Regla irrevocable 1.
...
SELECT j.tenant_id,
       ...
       j.state,
       e.currency_code,
       SUM(CASE WHEN e.side = a.normal_side THEN e.amount ELSE -e.amount END)::BIGINT AS saldo
  FROM ledger.entry   e
...
SELECT tenant_id, merchant_id, currency_code,
       SUM(saldo)::BIGINT AS saldo_dispersable
  FROM ledger.v_saldo
...
SELECT s.tenant_id,
       ...
       a.name AS cuenta,
       s.currency_code,
       SUM(s.saldo)::BIGINT AS saldo_abierto
