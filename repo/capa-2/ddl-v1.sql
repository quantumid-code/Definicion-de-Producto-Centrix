-- =============================================================================
-- BLOQUE DE REVISION - 9 de septiembre de 2026 - v0.2
-- Trazabilidad: CONTROL-CAMBIOS.md, fila ddl-v1.sql
--
-- EL CUERPO EJECUTADO NO SE MODIFICA. Este DDL corrio contra PostgreSQL 16 y sus
-- invariantes se probaron. La regla 4 del repositorio prohibe subir SQL que no
-- haya corrido, asi que los cambios de abajo se aplican y se vuelven a ejecutar
-- en una sesion dedicada, no se escriben a ciegas.
--
-- INSUMOS: Prosa Especificacion Online Servicios de Agregadores 3.8.0
-- (24/02/2026) - Prosa Estandar Host POS Adquirente 8.12.0 - E-Global Anexo A 9.0
--
-- PENDIENTE 1 - REFERENCIA COLGADA. El COMMENT ON de la tabla de sub-afiliacion
--   cita la decision D43, que no existe en la copia vigente de
--   capa-0/0.3-registro-decisiones.md. Se conserva; se valida al reemplazar 0.3.
--
-- PENDIENTE 2 - ESCENARIO DE AGREGACION EN LA AFILIACION. Atributo versionado
--   con vigencia, no columna simple: determina la tarifa de toda la cartera que
--   cuelga de la afiliacion, y las evaluaciones anteriores deben conservar el
--   escenario que estuvo vigente. Ver 0.6 seccion 4.
--
-- PENDIENTE 3 - ADQUIRENTE PATROCINADOR. Identifica la multiadquirencia y viaja
--   en el mensaje. La conciliacion y la liquidacion deben poder cortar por el.
--   Ver 0.10 seccion 2.2.
--
-- PENDIENTE 4 - IDENTIFICADOR DE FACILITADOR DE PAGOS. Tabla propia colgada de
--   la afiliacion, uno por marca, con estados no_tramitado / en_tramite /
--   vigente / revocado. NO es columna de la afiliacion: tiene emisor distinto
--   (la marca), cardinalidad por marca y revocacion unilateral. Ver 0.2 4.2.
--
-- PENDIENTE 5 - ESTADO CRIPTOGRAFICO DE TERMINAL. Eje independiente del ciclo de
--   vida, con contador de uso. Restriccion: una terminal sin inicializar no
--   emite. Invariante I16. Ver 0.2 seccion 4.5.
--
-- PENDIENTE 6 - MODO DE CAPTURA Y FICHERO DE COMPENSACION. Tabla de fichero con
--   estados en_preparacion / emitido / confirmado / rechazado /
--   parcialmente_aceptado, y relacion con las capturas diferidas que contiene.
--   Invariante I17. Ver 0.2 seccion 5.1b.
--
-- PENDIENTE 7 - CATEGORIA COMERCIAL CON VIGENCIA. La tarifa NO se indexa por
--   codigo de giro: el mismo giro aparece en familias tarifarias distintas.
--   Invariante C9. El giro pasa a ser atributo de la categoria, no llave.
--   Ver 0.6 seccion 5.2.
--
-- PENDIENTE 8 - CATEGORIA DE RECHAZO en el resultado del intento. Invariante I18.
--
-- PENDIENTE 9 - IMPORTE COMPUESTO. Total con lista de componentes, cada uno con
--   concepto, importe y beneficiario, y verificacion de suma exacta. En Fase 0
--   tendra un solo renglon por transaccion; modelarlo despues obliga a migrar el
--   historico. Invariante M8. Ver 0.5 seccion 6.
--
-- PENDIENTE 10 - PRECISION UNIFORME. Verificar que ningun par de importes del
--   mismo calculo se persista con precisiones distintas. Invariante M9.
--
-- NO SE TOCA: la representacion monetaria como entero mas codigo de moneda en
-- columnas separadas es correcta y se conserva.
-- =============================================================================
...
CREATE TABLE merchants.affiliation (
    ...
    declared_status     TEXT NOT NULL
        CHECK (declared_status IN ('en_alta','activa','suspendida','cancelada')),
...
CREATE INDEX entry_por_cuenta ON ledger.entry (account_code, journal_period);
