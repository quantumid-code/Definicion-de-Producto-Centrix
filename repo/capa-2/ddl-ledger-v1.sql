...
-- Trazabilidad: CONTROL-CAMBIOS.md, fila ddl-ledger-v1.sql
--
-- EL CUERPO EJECUTADO NO SE MODIFICA. Ver la nota equivalente en ddl-v1.sql.
--
-- INSUMO: Prosa Especificacion Online Servicios de Agregadores 3.8.0, subcampo
-- de captura diferida del elemento de liquidacion.
--
-- PENDIENTE 1 - CUENTA DE CONTROL "CAPTURAS PENDIENTES DE COMPENSAR". Es la
--   adicion mas importante de esta revision. Su saldo es dinero que el comercio
--   cree cobrado, que Centrix registro como capturado, y que nadie ha compensado
--   todavia. A diferencia de las otras cuentas de control, no crece por un error
--   del sistema: crece por una omision que NO PRODUCE NINGUNA ALARMA EXTERNA.
--   Ver 0.7 seccion 3.
--
-- PENDIENTE 2 - ASIENTOS NUEVOS. Captura en modo diferido, fichero de
--   compensacion confirmado, fichero de compensacion rechazado. La captura
--   diferida NO comparte asiento con la integrada: si lo hiciera, el libro
--   afirmaria una cuenta por cobrar al procesador cuando el procesador todavia
--   no sabe nada de la transaccion. Ver 0.7 seccion 4.1.
--
-- PENDIENTE 3 - TERCERA CONDICION DE BLOQUEO DE DISPERSION. Lo capturado en modo
--   diferido y no compensado no se dispersa. Sale del mismo mecanismo de cuenta
--   de control que las otras dos, sin cambiar el mecanismo. Invariante I17 /
--   L10. Ver 0.7 seccion 7.
--
-- PENDIENTE 4 - CONDICION DE CIERRE DE PERIODO. Un periodo no se cierra con
--   saldo en capturas pendientes de compensar sin decision registrada.
--   Invariante L11. Ver 0.7 seccion 6.
--
-- PENDIENTE 5 - ASIENTOS DE FLUJO DE EMPUJE. La emision de referencia no genera
--   asiento; la confirmacion de pago genera el equivalente a captura; no hay
--   asiento de autorizacion. Ver 0.9 seccion 8. Fase 1, no bloquea Fase 0.
-- =============================================================================
