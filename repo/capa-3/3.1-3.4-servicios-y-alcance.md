> **Bloque de revisión · 9 de septiembre de 2026 · v0.2 · PRIORIDAD ALTA.**
> Trazabilidad en `CONTROL-CAMBIOS.md`, fila 3.1–3.4.
>
...
>
> **Este documento define el alcance de cumplimiento, y ese alcance tiene hoy una zona sin
> resolver.** No se regenera en esta versión porque hacerlo exigiría fijar un perímetro sobre
> una lectura que aún no está confirmada, y un perímetro mal fijado es peor que uno pendiente.
>
> **Lo que sí quedó verificado.** En el mensaje de autorización el contenido de pista **no
> viaja en el campo**: viaja cifrado en una estructura de token, y el manual atribuye el cambio
> a normativa del regulador bancario mexicano. La decisión D1b se sostiene ahí por mandato
> regulatorio, no por acuerdo comercial. La excepción de envío en claro existe pero es opt-in,
> sujeta a visto bueno expreso del área de seguridad de la cámara, y Centrix no la solicita.
>
> **Lo que no está resuelto · Q-1.** Para el mensaje de aviso el manual admite envío de pista
> recortada, que contiene el número de cuenta completo, y la especificación de agregadores
> afirma que esos campos quedan protegidos por el cifrado del canal. **Las dos lecturas dan
> perímetros distintos**, y de ellas depende si el adaptador queda dentro o fuera del alcance
> para número de cuenta. Es la pregunta más importante del paquete.
>
> **Pendiente adicional · superficie del intercambio de llaves.** El método de protección de
> llaves es mandatorio en cámara y su intercambio ocurre por la propia mensajería. Eso añade al
> perímetro un flujo que este documento no contempla. Ver 0.2 §4.5 y el documento 0.11.
...
1. **Compuerta E3.** Determina si el apartado 12 aplica.
2. **Si la pantalla hospedada del procesador cubre los casos de uso del primer cliente.** Si no,
   la alternativa del apartado 5 se vuelve el camino principal.
3. **Contratación del evaluador calificado** y análisis de brecha sobre esta arquitectura. Mes 2 a 3.
   Contratarlo tarde convierte cada hallazgo en un rediseño.
4. **Prueba de segmentación:** herramienta, periodicidad y quién la ejecuta.
5. **Clasificación del `terminal-gateway`** por el evaluador. La posición de Centrix está sostenida;
